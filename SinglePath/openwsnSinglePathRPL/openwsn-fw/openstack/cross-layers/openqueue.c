...
OpenQueueEntry_t* openqueue_macGetDataPacket(open_addr_t* toNeighbor) {
   ...
   if (toNeighbor->type==ADDR_64B) {
      ...
      for (i=0;i<QUEUELENGTH;i++) {
         /*// erfan debugging
         if (openqueue_vars.queue[i].l4_protocol==IANA_UDP)
            printf("Queue=>currMote: %d, timeSlot:%d, creator:%d, l4_proto:%d, src:%d, nextHop:%d, l2_dsn:%d, attempts:%d\n", 
               idmanager_getMyID(ADDR_64B)->addr_64b[7], schedule_getSlotOffset(),
               openqueue_vars.queue[i].creator, openqueue_vars.queue[i].l4_protocol, 
               openqueue_vars.queue[i].l3_sourceAdd.addr_128b[15], 
               openqueue_vars.queue[i].l2_nextORpreviousHop.addr_64b[7], openqueue_vars.queue[i].l2_dsn, 
               openqueue_vars.queue[i].l2_numTxAttempts);
         */                                                                       // added by Erfan to pass only uinject packets
