...
# SinglePath reads schedule from this file
SinglePathinput='scheduling_motes.txt'
# SinglePath output file name
SinglePathoutput='finalStat.txt'
# SinglePath reads start of each flow from this file
SinglePathstartOfFlows='startOfFlows.txt'
# BraidedPaths reads list of crashed motes from this file
SinglePathCrashedMotes='crashed_motes.txt'
#topologyGenerator.py and extractDODAG-fw(sw).py folder
stats_plots='stats_plots'
# creating folders
dirName=['results/topology_Dodag_Schedule', 'results/SinglePathresults', 'results/plots',
		 'openwsnSinglePathRPL/logs_Topology']
...
# list of directories in 'results/SinglePathresults'
...
if runModules['calcSchedule'] == 'y':
	...
	for dodag in dodag_logs:
		inputFile = '../{0}/{1}'.format(dirName[0], dodag)
...
if runModules['runOpenWSN'] == 'y':
	# removes files of openwsnSinglePathRPL/logs_Topology
	for the_file in os.listdir(dirName[3]):
	    os.remove(os.path.join(dirName[3], the_file))

	# remove openvisualizer log file
	if os.path.exists('openwsnSinglePathRPL/openwsn-sw/software/openvisualizer/openwsnLogs.txt'):
		os.remove('openwsnSinglePathRPL/openwsn-sw/software/openvisualizer/openwsnLogs.txt')

	# remove previous files in results/SinglePathresults
	...
	# copy topologies from results/topology_Dodag_Schedule to openwsnSinglePathRPL/logs_Topology
	for the_file in os.listdir(dirName[0]):
		shutil.copy2(os.path.join(dirName[0], the_file), dirName[3])

	#  current Dir: openwsnSinglePathRPL/logs_Topology 
	os.chdir(dirName[3])
	...
	for top in topos:
		...
		os.rename(scheduling, SinglePathinput)
		# rename start of flow file to 'startOfFlows.txt'
		os.rename(startOfFlow, SinglePathstartOfFlows)
		...
		if os.path.exists('../../{0}/{1}'.format(dirName[0], crashedMotes)):
			...
			os.rename(crashedMotes, SinglePathCrashedMotes)

		# reads 'scheduling_motes.txt' and topology with duration and runs openwsnSinglePathRPL
		...
		os.rename(SinglePathinput, scheduling)
		# rename 'startOfFlows.txt' to start of flow file name
		os.rename(SinglePathstartOfFlows, startOfFlow)
		...
		if os.path.exists(crashedMotes):
			# rename crashed motes file to 'crashedMotes.txt'
			os.rename(SinglePathCrashedMotes, crashedMotes)
