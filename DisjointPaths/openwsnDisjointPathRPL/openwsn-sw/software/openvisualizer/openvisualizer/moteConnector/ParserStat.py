...
class ParserStat(Parser.Parser):
 ...
 #returns a string with the decimal value of a uint16_t
    ...
    def LogPktTx(self, addr, asnbytes, input, message):
        log.info('{7} |addr= {0} |asn= {1} |length= {2} |slotOffset= {3} |frequency= {4} |l2Dest= {5} |numTxAttempts= {6} |L3Src= {8} |L3Dest= {9} |L4Proto= {10} |SeqNum= {11} |l2Dsn= {12} |frameType= {13} |txpower= {14}'.format(
            self.BytesToAddr(addr),
            self.BytesToString(asnbytes),
            input[8],
            self.BytesToString(input[10:12]),
            input[12],
            self.BytesToAddr(input[20:21]),
            input[22],
            message,
            self.BytesToAddr(input[38:39]),
            self.BytesToAddr(input[54:55]),
    ...
    def LogPktRx(self, addr, asnbytes, input, message):
        log.info('{6} |addr= {0} |asn= {1} |length= {2} |slotOffset= {3} |frequency= {4} |l2Src= {5} |L3Src= {7} |L3Dest= {8} |L4Proto= {9} |SeqNum= {10} |l2Dsn= {11} |frameType= {12}, |rssi= {13} |lqi= {14} |crc= {15}'.format(
            self.BytesToAddr(addr),
            self.BytesToString(asnbytes),
            input[8],
            self.BytesToString(input[10:12]),
            ...
            self.BytesToAddr(input[20:21]),
            message,
            self.BytesToAddr(input[39:40]),
            self.BytesToAddr(input[55:56]),
            self.ByteToL4protocol(input[56]),
            self.BytesToString(input[57:59]),
            input[59],
            self.ByteToFrameType(input[9]),
            input[21],
            input[22],
