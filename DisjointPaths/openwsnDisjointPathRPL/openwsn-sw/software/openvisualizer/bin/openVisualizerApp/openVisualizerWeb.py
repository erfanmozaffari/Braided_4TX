...
if __name__=="__main__":
    ...
    # added by erfan to set simulation time
    if (argspace.duration>0):
        banner  = []
        banner += ['\nOpenVisualizer']
        banner += ['web interface started at {0}:{1}'.format(argspace.host,argspace.port)]
        banner += ['simulation will finish {0} seconds later'.format(argspace.duration)]
        banner  = '\n'.join(banner)
        print  banner
        now = time.time()
        endTime = now + argspace.duration
        while True:
            if time.time() > endTime:
                print '\n Simulation Time finished.(Duration: {0} seconds).'.format(argspace.duration)
                webapp.do_quit(1)
                #app.close()
                #os.kill(os.getpid(), signal.SIGTERM)
    # end of erfan !!
