...
//===== Statistics - aded by Erfan, Fab's stat (all the stats)
...
void openserial_statAck(uint8_t code, open_addr_t *l2addr){
    ...
    if (getLogs != NULL) {
        if (evt.code==2) {
            fprintf(getLogs, "STAT_ACK |addr= %d |asn= %d |status= TXED |l2addr= %d\n",                                                                       
                            idmanager_getMyID(ADDR_64B)->addr_64b[7], currentASN, l2addr->addr_64b[7]);
        }
        else{
            fprintf(getLogs, "STAT_ACK |addr= %d |asn= %d |status= RCVD |l2addr= %d\n",                                                                       
                            idmanager_getMyID(ADDR_64B)->addr_64b[7], currentASN, l2addr->addr_64b[7]);
        }
    }
    fclose(getLogs);
...
void openserial_statRx(OpenQueueEntry_t* msg){
   ...
   evt.l2Dsn 					   = msg->l2_dsn;
   if (msg->l4_protocol==IANA_UDP){
   	uint8_t uinject_payload[] = "uinject";
   	uint8_t i;
   	
   	for (i=sizeof(msg->packet)-1;i>8;i--){
   		// checks to find position of "uinject_payload[]" and whether sender ID in packet and current sender is the same or not
   		if ((msg->packet[i-6]==uinject_payload[0]) && (msg->packet[i-5]==uinject_payload[1]) && 
   			 (msg->packet[i-4]==uinject_payload[2]) && (msg->packet[i-3]==uinject_payload[3]) &&
   			 (msg->packet[i-2]==uinject_payload[4]) && (msg->packet[i-1]==uinject_payload[5]) &&
   			 (msg->packet[i]==uinject_payload[6]) && (msg->packet[i-9]==msg->l3_sourceAdd.addr_128b[15])
   		){
   			evt.sequenceNumber[0] = msg->packet[i-8]; // 1st byte
   			evt.sequenceNumber[1] = msg->packet[i-7]; // 2nd byte
   ...
   if (getLogs != NULL) {
        if (msg->l4_protocol==IANA_UDP) {
            fprintf(getLogs, "STAT_PK_RX |addr= %d |asn= %d |length= %d |slotOffset= %d |frequency= %d |l2Src= %d |L3Src= %d |L3Dest= %d |L4Proto= IANA_UDP |SeqNum= %d |l2Dsn= %d |frameType= %d |rssi= %d |lqi= %d |crc= %d\n", 
                            idmanager_getMyID(ADDR_64B)->addr_64b[7], currentASN, evt.length, evt.slotOffset, evt.frequency, 
                            msg->l2_nextORpreviousHop.addr_64b[7], msg->l3_sourceAdd.addr_128b[15], msg->l3_destinationAdd.addr_128b[15],
                            evt.sequenceNumber[1]*255+evt.sequenceNumber[0], evt.l2Dsn, evt.frame_type, evt.rssi, evt.lqi, evt.crc);
       }
       else{
            fprintf(getLogs, "STAT_PK_RX |addr= %d |asn= %d |length= %d |slotOffset= %d |frequency= %d |l2Src= %d |L3Src= %d |L3Dest= %d |L4Proto= %d |SeqNum= %d |l2Dsn= %d |frameType= %d |rssi= %d |lqi= %d |crc= %d\n", 
                            idmanager_getMyID(ADDR_64B)->addr_64b[7], currentASN, evt.length, evt.slotOffset, evt.frequency, 
                            msg->l2_nextORpreviousHop.addr_64b[7], msg->l3_sourceAdd.addr_128b[15], msg->l3_destinationAdd.addr_128b[15], 
                            msg->l4_protocol, evt.sequenceNumber[1]*255+evt.sequenceNumber[0], evt.l2Dsn, evt.frame_type, evt.rssi, evt.lqi, evt.crc);
       }
   }
   fclose(getLogs);
...
void openserial_statTx(OpenQueueEntry_t* msg){
   ...
   memcpy(evt.l3_sourceAdd,        msg->l3_sourceAdd.addr_128b, 16);
    ...
    if (getLogs != NULL) {
        if (msg->l4_protocol==IANA_UDP) {
            fprintf(getLogs, "STAT_PK_TX |addr= %d |asn= %d |length= %d |slotOffset= %d |frequency= %d |l2Dest= %d |numTxAttempts= %d |L3Src= %d |L3Dest= %d |L4Proto= IANA_UDP |SeqNum= %d |l2Dsn= %d |frameType= %d |txpower= %d\n",                                                                       
                            idmanager_getMyID(ADDR_64B)->addr_64b[7], currentASN, evt.length, evt.slotOffset, evt.frequency, 
                            msg->l2_nextORpreviousHop.addr_64b[7], evt.numTxAttempts, msg->l3_sourceAdd.addr_128b[15], 
                            msg->l3_destinationAdd.addr_128b[15], evt.sequenceNumber[1]*255+evt.sequenceNumber[0], evt.l2Dsn, 
                            evt.frame_type, evt.txPower);
        }
        else{
            fprintf(getLogs, "STAT_PK_TX |addr= %d |asn= %d |length= %d |slotOffset= %d |frequency= %d |l2Dest= %d |numTxAttempts= %d |L3Src= %d |L3Dest= %d |L4Proto= %d |SeqNum= %d |l2Dsn= %d |frameType= %d |txpower= %d\n",                                                                       
                            idmanager_getMyID(ADDR_64B)->addr_64b[7], currentASN, evt.length, evt.slotOffset, evt.frequency, 
                            msg->l2_nextORpreviousHop.addr_64b[7], evt.numTxAttempts, msg->l3_sourceAdd.addr_128b[15], 
                            msg->l3_destinationAdd.addr_128b[15], evt.l4_protocol, evt.sequenceNumber[1]*255+evt.sequenceNumber[0], 
                            evt.l2Dsn, evt.frame_type, evt.txPower);
        }
    }
   fclose(getLogs);
