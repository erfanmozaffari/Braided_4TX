...
void icmpv6rpl_init() {
   ...
   icmpv6rpl_vars.secondPreferredParent=FALSE;  // set default as not found, added by Erfan for secondary parent
  #endif
...
#ifdef SECONDARY_PARENT
// *** function added by Erfan to get second preferred parent *** //
/**
\brief Retrieve my preferred parent's EUI64 address.
\param[out] addressToWrite Where to copy the preferred parent's address to.
*/
bool icmpv6rpl_getSecondPreferredParentEui64(open_addr_t* addressToWrite) {
    if (
        // second condition is not needed as was checked for first preferred parent 
        icmpv6rpl_vars.secondPreferredParent  &&
        neighbors_getNeighborNoResource(icmpv6rpl_vars.secondParentIndex)==FALSE
    ){
  #ifdef MCS_DEBUG  
        printf("2ndPrefIcmp; secondPrefParent found\n"); //debugging added by Erfan for secondary parent
  #endif
...
/*#ifdef SECONDARY_PARENT
    bool      secondBetterFound;       // if second one found becomes true, added by Erfan for secondary parent
    // temporaries
    dagrank_t secondNeighborRank;      // temporary variable to keep rdag ank of neighbors, added by Erfan for secondary parent
    uint8_t   secondParentIndex;       // temporary variable to keep index of second pref parent, added by Erfan for secondary parent
    uint16_t  secondDAGrank;           // temporary variable to keep dag rank of second pref parent, added by Erfan for secondary parent
#endif*/
    ...
    icmpv6rpl_vars.haveParent = FALSE;
/*#ifdef SECONDARY_PARENT
    secondBetterFound    = FALSE;                    // Initialized, added by Erfan for secondary parent
    secondDAGrank        = icmpv6rpl_vars.myDAGrank; // Initialized, added by Erfan for secondary parent
    secondNeighborRank   = icmpv6rpl_vars.myDAGrank; // Initialized, added by Erfan for secondary parent
#endif*/
    ...
    for (i=0;i<MAXNUMNEIGHBORS;i++) {
      ...
      /*#ifdef  DISJOINT_PATH_DEBUG
            printf("UpdateDagRank; addr: %d, prev: %d, tent: %d, rankInc: %d, neighborRank: %d\n", 
                    idmanager_getMyID(ADDR_64B)->addr_64b[7], previousDAGrank, 
                    tentativeDAGrank, rankIncrease, neighborRank
                  ); // debugging by Erfan for secondary parent
      #endif*/
            if (
              ...
              /*#ifdef SECONDARY_PARENT
                  // if no better parent found then assign it here, added by Erfan for secondary parent
                  if (!foundBetterParent && neighborRank<secondNeighborRank && i!=icmpv6rpl_vars.ParentIndex) {
                      if (secondDAGrank-tentativeDAGrank>=2*MINHOPRANKINCREASE) {
                          secondBetterFound  = TRUE;
                          secondNeighborRank = neighborRank;
                          secondDAGrank      = tentativeDAGrank;
                          secondParentIndex  = i; 
                      }
                  }
              #endif*/
            ...
            if (icmpv6rpl_vars.myDAGrank>tentativeDAGrank) {
              /*#ifdef SECONDARY_PARENT
                // if better parent found, previous one becomes index of 2nd pref parent, added by Erfan for secondary parent
                icmpv6rpl_vars.secondParentIndex = icmpv6rpl_vars.ParentIndex;
              #endif*/
                icmpv6rpl_vars.myDAGrank         = (uint16_t)tentativeDAGrank;
   ...
   if (foundBetterParent) {
      ...
      if (!prevHadParent) {
         ...
         //neighbors_setPreferredParent(prevParentIndex, FALSE);     // commented by Erfan to enable setting parents statically
         // set neighbors as preferred parent
         //neighbors_setPreferredParent(icmpv6rpl_vars.ParentIndex, TRUE);    // commented by Erfan to enable setting parents statically
      } else {
         if (icmpv6rpl_vars.ParentIndex==prevParentIndex) {
          ...
          /*#ifdef SECONDARY_PARENT
             // second best parent is found now, added by Erfan for secondary parent
             icmpv6rpl_vars.secondPreferredParent =TRUE; 
             // second best parent index, added by Erfan for secondary parent
             icmpv6rpl_vars.secondParentIndex     = prevParentIndex;
          #endif*/
      ...
      /*
    #ifdef SECONDARY_PARENT
      // choosing 2nd pref parent added by Erfan for secondary parent
      if (secondBetterFound) {
          icmpv6rpl_vars.secondPreferredParent  = TRUE;
          icmpv6rpl_vars.secondParentIndex      = secondParentIndex;
      }
      // choosing 2nd pref parent end by Erfan for secondary parent
    #endif*/
   }
#ifdef SECONDARY_PARENT   
   schedule_getParents(&(icmpv6rpl_vars.haveParent),&(icmpv6rpl_vars.ParentIndex),&(icmpv6rpl_vars.secondPreferredParent),&(icmpv6rpl_vars.secondParentIndex));
#endif 
}
