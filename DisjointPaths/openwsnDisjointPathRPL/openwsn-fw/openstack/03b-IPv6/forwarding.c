...
void forwarding_receive(
    ...
    if (
        ...
        if (ipv6_outer_header->next_header!=IANA_IPv6ROUTE) {
            ...
            }/* commented by Erfan to avoid loop detection for statically set parents
...
/** function added by Erfan for rpl to return second pref parent as next hop
\brief Retrieve the next hop's address from routing table.

\param[in]  destination128b  Final IPv6 destination address.
\param[out] addressToWrite64b Location to write the EUI64 of next hop to.
*/
bool forwarding_getSecondPrefParentAsNextHop(open_addr_t* destination128b, open_addr_t* addressToWrite64b) {
   uint8_t         i;
   open_addr_t     temp_prefix64btoWrite;
   bool            isSecondPreferredParent=FALSE;
   ...
   if (packetfunctions_isBroadcastMulticast(destination128b)) {
      ...
      // destination is remote, send to preferred parent
      isSecondPreferredParent=icmpv6rpl_getSecondPreferredParentEui64(addressToWrite64b);
   }
   return isSecondPreferredParent;
}
// function end by Erfan for rpl to return second preferred parent as next hop
#endif
...
owerror_t forwarding_send_internal_RoutingTable(
...
#ifdef DISJOINT_PATH
   OpenQueueEntry_t* replicaPkt;
   // sends replica of packet to lower layer added by Erfan for rpl
   ...
   if (msg->l2_nextORpreviousHop.addr_64b[7] == msg->l3_destinationAdd.addr_128b[15]) {
   	if (msg->l2_nextORpreviousHop.type != ADDR_NONE && msg->creator == COMPONENT_UINJECT && PCKTSEND == fw_SendOrfw_Rcv) {
		   replicaPkt = openqueue_getFreePacketBuffer(COMPONENT_UINJECT);
		   if (replicaPkt!=NULL) {
		#ifdef DISJOINT_PATH_DEBUG
		      printf("duplicate primary parent for 1 hop neighbors\n");
		      printf("before iphc_sendfomfw dup pkt\n");
		#endif
		      // make a local copy of packet
		      packetfunctions_duplicatePacket(replicaPkt, msg);
		      // send replicated packet to next lower layer          
		      if (iphc_sendFromForwarding(
		          replicaPkt,
		          ipv6_outer_header,
		          ipv6_inner_header,
		          rpl_option,
		         ...
		         openqueue_freePacketBuffer(replicaPkt);
		      }
			}
		}
   }
   else if (msg->l2_nextORpreviousHop.addr_64b[7] != msg->l3_destinationAdd.addr_128b[15]) {
		if ((msg->creator == COMPONENT_UINJECT) && (PCKTSEND == fw_SendOrfw_Rcv) && 
		  forwarding_getSecondPrefParentAsNextHop(&(msg->l3_destinationAdd),&(msg->l2_nextORpreviousHop))) {
			if (msg->l2_nextORpreviousHop.type!=ADDR_NONE) {
			   replicaPkt = openqueue_getFreePacketBuffer(COMPONENT_UINJECT);
			   if (replicaPkt!=NULL) {
			#ifdef DISJOINT_PATH_DEBUG
			      printf("have second parent\n");
			      printf("before iphc_sendfomfw dup pkt\n");
			#endif
			      // make a local copy of packet
			      packetfunctions_duplicatePacket(replicaPkt, msg);
			...
			#ifdef DISJOINT_PATH_DEBUG  
			   printf("after iphc_sendfomfw dup pkt\n");
			   printf("second prefPar src addr: %d, prefParent: %d, dest: %d\n", replicaPkt->l3_sourceAdd.addr_128b[15], replicaPkt->l2_nextORpreviousHop.addr_64b[7], replicaPkt->l3_destinationAdd.addr_128b[15]);
			#endif 
			  }
			  else {
			    	printf("\nnot enough queue\n\n");
   ...
   // sends replica of packet to lower layer end by Erfan for rpl
#endif

   // retrieve the next hop from the routing table
   forwarding_getNextHop(&(msg->l3_destinationAdd),&(msg->l2_nextORpreviousHop));
   if (msg->l2_nextORpreviousHop.type==ADDR_NONE) {
      // added by Erfan to reconverge the traffic
      if (!forwarding_getSecondPrefParentAsNextHop(&(msg->l3_destinationAdd),&(msg->l2_nextORpreviousHop))) {
         openserial_printError(
            COMPONENT_FORWARDING,
            ERR_NO_NEXTHOP,
            (errorparameter_t)0,
...
#ifdef DISJOINT_PATH_DEBUG  
   if (msg->creator == COMPONENT_UINJECT)
      printf("first prefPar src addr: %d, prefParent: %d, dest: %d\n", msg->l3_sourceAdd.addr_128b[15], msg->l2_nextORpreviousHop.addr_64b[7], msg->l3_destinationAdd.addr_128b[15]);
#endif 
