...
parents_vars_t parents_vars; // added by Erfan to save primary and secondary parents
...
void schedule_init() {
    ...
    // loads scheduling from text file
    schedule_loadScheduling(SCHEDULE_MINIMAL_6TISCH_ACTIVE_CELLS+NUMSERIALRX);
#endif
#ifdef CRASH_NODES
    // set which nodes to crash on what ASN, added by Erfan
    schedule_loadCrashStat();
#endif
    // serial RX slot(s)
...
#ifdef STATIC_SCHEDULE
/**
\breif Loading external scheduling, added by Erfan
*/
void schedule_loadScheduling(uint8_t startingSlotOffset) {

   uint8_t     moteId, count, bufsize=0, numOfCells;
   uint32_t    cellInfo[5];
   bool        found=FALSE;
   char        *buffer,*p;
   open_addr_t neighborAddress;

   FILE *file = fopen ("../../../logs_Topology/scheduling_motes.txt", "r");
   //FILE *file = fopen ("scheduling_motes.txt", "r");
   if ( file != NULL )
   {
      // read titles
      ...
      while(!found)
      {
         ...
         numOfCells = atoi(p);
         if (p=strtok(NULL, " ")) {
             memset(&parents_vars.primaryParent,0,sizeof(parents_vars.primaryParent));
             parents_vars.primaryParent.type        = ADDR_64B;
             parents_vars.primaryParent.addr_64b[0] = 0x14;
             parents_vars.primaryParent.addr_64b[1] = 0x15;
             parents_vars.primaryParent.addr_64b[2] = 0x92;
             parents_vars.primaryParent.addr_64b[3] = 0xcc;
             parents_vars.primaryParent.addr_64b[4] = 0x00;
             parents_vars.primaryParent.addr_64b[5] = 0x00;
             parents_vars.primaryParent.addr_64b[6] = 0x00;
             parents_vars.primaryParent.addr_64b[7] = atoi(p);
         }
         else {
             memset(&parents_vars.primaryParent,0,sizeof(parents_vars.primaryParent));
             parents_vars.primaryParent.type = ADDR_NONE;
         }
         if (p=strtok(NULL, " ")) {
             memset(&parents_vars.secondaryParent,0,sizeof(parents_vars.secondaryParent));
             parents_vars.secondaryParent.type        = ADDR_64B;
             parents_vars.secondaryParent.addr_64b[0] = 0x14;
             parents_vars.secondaryParent.addr_64b[1] = 0x15;
             parents_vars.secondaryParent.addr_64b[2] = 0x92;
             parents_vars.secondaryParent.addr_64b[3] = 0xcc;
             parents_vars.secondaryParent.addr_64b[4] = 0x00;
             parents_vars.secondaryParent.addr_64b[5] = 0x00;
             parents_vars.secondaryParent.addr_64b[6] = 0x00;
             parents_vars.secondaryParent.addr_64b[7] = atoi(p);
         }
         else {
             memset(&parents_vars.secondaryParent,0,sizeof(parents_vars.secondaryParent));
             parents_vars.secondaryParent.type = ADDR_NONE;
         }
        /*printf("\nprimaryParent: ");
        uint8_t j=0;
        for (j=0;j<8; j++)
                  printf("%d ",parents_vars.primaryParent.addr_64b[j]);
        printf("\nsecondaryParent: ");
        for (j=0;j<8; j++)
                  printf("%d ",parents_vars.secondaryParent.addr_64b[j]);
        ...
        printf("\n\n");*/
         ...
         if (moteId == idmanager_getMyID(ADDR_64B)->addr_64b[7])
         {
            while (count<numOfCells)
            {
               ...
               if (cellInfo[4]==0) {
                  neighborAddress.type        = ADDR_ANYCAST;
               }
               else {
                  ...
                  neighborAddress.addr_64b[7] = cellInfo[4];
               }
               //                         slot slotOffset            type of slot    shared?   channel offset    neighobor
               schedule_addActiveSlot(startingSlotOffset+cellInfo[0], cellInfo[1], cellInfo[2], cellInfo[3], &neighborAddress);
               count++;
            }
            found=TRUE;
         }
         ...
         {
           while (count<numOfCells) {/* read a line */
              getline(&buffer,&bufsize,file);
              count++;
            }
         }
...
#ifdef CRASH_NODES
/**
\breif Load CRASH_ID & CRASH_ASN, added by Erfan
*/
void schedule_loadCrashStat() {
   uint8_t count,numNodes,nodeID;
   uint32_t crashASN;

   FILE *file = fopen ("../../../logs_Topology/crashed_motes.txt", "r");
   if ( file != NULL )
   {
      ...
      {  
         fscanf(file,"%d",&crashedNodes_ID[count]);
         fscanf(file,"%d",&crashASN);
         crashedNodes_ASN[count] = crashASN;
         count++;
      }
      fclose (file);
   }
...
#ifdef SECONDARY_PARENT
/**
\breif set primary and secondary parents statically, added by Erfan
...
- set preferred parent based on schedule
- havePrimary?
- primaryIndex in neighbor table
- haveSecondary?
- secondaryIndex in neighbor table

\param[in] havePrimary, primaryIndex, haveSecondary, secondaryIndex
   header.
*/
               ...
               *primaryIndex=i;
               ...
               if (icmpv6rpl_getPreferredParentEui64(&address)) {
                  if (!packetfunctions_sameAddress(&address, &parents_vars.primaryParent)) {
                     ...
                     neighbors_setPreferredParent(j, FALSE);
                     neighbors_setPreferredParent(i, TRUE);
                  }
               }
               else
                  neighbors_setPreferredParent(i, TRUE);
               ...
               if (isThisRowMatching(&parents_vars.secondaryParent,i)) {
                  ...
                  *primaryIndex=i;
                  ...
                  if (icmpv6rpl_getPreferredParentEui64(&address)) {
                     if (packetfunctions_sameAddress(&address, &parents_vars.primaryParent)) 
                        break;
                     if (!packetfunctions_sameAddress(&address, &parents_vars.secondaryParent)) {
                        icmpv6rpl_getPreferredParentIndex(&j);
                        neighbors_setPreferredParent(j, FALSE);
                        neighbors_setPreferredParent(i, TRUE);
                     }
...
owerror_t schedule_addActiveSlot(
   ...
   openserial_statCell(CELLADD, slotContainer); // added by Erfan, Fab's stat
...
owerror_t schedule_removeActiveSlot(slotOffset_t slotOffset, open_addr_t* neighbor) {
   ...
   openserial_statCell(CELLDEL, slotContainer); // added by Erfan, Fab's stat
