...
def main(logPath, destPath, slotDuration, numMinimalCells):
    ...
    stat_data = infile.readlines()
    infile.close()
    finalPath = destPath + "finalStat_onlyDataPkt.txt"
    ...
    stat_data = infile.readlines()
    infile.close()
    finalPath = destPath + "finalStatSorted_noDup.txt"
    outfile    = open(finalPath, 'w')
    ...
    finalPath = destPath + "finalStatSorted.txt"
    rmvDupsHandler = open(finalPath, 'w')
    ...
    finalPath = destPath + "finalStatSorted.txt"
    with open(finalPath) as rcvdPkts:
        finalPath             = destPath +  "pkts_snt_rcvd_time.txt"
        sentReceived_timeFile = open(finalPath, 'w')
        ...
        for rcvdLine in rcvdPkts:
            ...
            if (x[0] == 'STAT_PK_RX') and (int(x[2],16) == sinkID) and x[18] == 'IANA_UDP':
                finalPath  = destPath + "finalStatSorted.txt"
                senderASN, numTxAttempt, tempPath = getSentAndReceivedTime(rcvdLine, finalPath)
    ...
    finalPath = destPath + "pkts_snt_rcvd_time.txt"
    sentReceived_File = open(finalPath, 'a')
    ...
    finalPath       = destPath + "delay_pdr.txt"
    delayPdrHandler = open(finalPath, 'w')
    delayPdrHandler.write("#Source NumSentPkts NumReceivedPkts PDR    TotalTime MinDelay MaxDelay AvgDelay LowerBound UpperBound HopDistance AvgJitter\n")
    for moteID in range(topologySize):
        if moteID + 1 != sinkID:
            ...
            if math.isnan(upperBound):
                upperBound = 0
            delayPdrHandler.write(" {:<7}{:<12}{:<16}{:<7.3f}{:<10}{:<9}{:<9}{:<9.2f}{:<11.2f}{:<11.2f}{:<12}{:<9.2f}\n".format(
                                                                                               moteID + 1,                                      # mote ID
    ...
    finalPath = destPath + "timeBasedPDR.txt"
    outfile    = open(finalPath, 'w')
