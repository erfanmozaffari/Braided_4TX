...
void icmpv6rpl_init() {
   ...
   icmpv6rpl_vars.secondPreferredParent=FALSE;  // set default as not found, added by Erfan for rpl
...
/** function added by Erfan for rpl to get second preferred parent
\brief Retrieve my preferred parent's EUI64 address.
\param[out] addressToWrite Where to copy the preferred parent's address to.
*/
...
// function ends by Erfan for rpl to get second preferred parent
...
void icmpv6rpl_updateMyDAGrankAndParentSelection() {
    ...
    bool      secondBetterFound;       // if second one found becomes true, added by Erfan for rpl
    // temporaries
    dagrank_t secondNeighborRank;      // temporary variable to keep rdag ank of neighbors, added by Erfan for rpl
    uint8_t   secondParentIndex;       // temporary variable to keep index of second pref parent, added by Erfan for rpl
    uint16_t  secondDAGrank;           // temporary variable to keep dag rank of second pref parent, added by Erfan for rpl
    ...
    icmpv6rpl_vars.haveParent = FALSE;
    secondBetterFound    = FALSE;                    // Initialized, added by Erfan for rpl
    secondDAGrank        = icmpv6rpl_vars.myDAGrank; // Initialized, added by Erfan for rpl
    secondNeighborRank   = icmpv6rpl_vars.myDAGrank; // Initialized, added by Erfan for rpl
        ...
        if (neighbors_isStableNeighborByIndex(i)) { // in use and link is stable
            ...
            if (
                ...
                (previousDAGrank-tentativeDAGrank < 2*MINHOPRANKINCREASE)
                  ...
                  // if no better parent found then assign it here, added by Erfan for rpl
            ...
            if (icmpv6rpl_vars.myDAGrank>tentativeDAGrank) {
                icmpv6rpl_vars.secondParentIndex = icmpv6rpl_vars.ParentIndex; // if better parent found, previous one becomes index of 2nd pref parent, added by Erfan for rpl
                icmpv6rpl_vars.myDAGrank         = (uint16_t)tentativeDAGrank;
   ...
   if (foundBetterParent) {
      ...
      if (!prevHadParent) {
         ...
         if (icmpv6rpl_vars.ParentIndex==prevParentIndex) {
             ...
             // second best parent is found now, added by Erfan for rpl
             icmpv6rpl_vars.secondPreferredParent = TRUE;
             // second best parent index, added by Erfan for rpl
      ...
      // choosing 2nd pref parent added by Erfan for rpl
      if (secondBetterFound) {
          icmpv6rpl_vars.secondPreferredParent  = TRUE;
          icmpv6rpl_vars.secondParentIndex      = secondParentIndex;
      }
      // choosing 2nd pref parent end by Erfan for rpl
   }
}
