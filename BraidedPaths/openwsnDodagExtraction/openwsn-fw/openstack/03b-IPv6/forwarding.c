...
owerror_t forwarding_send_internal_RoutingTable(
	...
	// get logs of best parent and second parent, added by Erfan
	FILE *firstAndSecondParent;
   firstAndSecondParent = fopen("../../../logs_Topology/parentContainer.txt","a");
   
   if (firstAndSecondParent != NULL) {
   	if (msg->creator == COMPONENT_UINJECT) {
         fprintf(firstAndSecondParent, "CurrentNode: %d ", idmanager_getMyID(ADDR_64B)->addr_64b[7]);
      }
   }
   ...
   if ((msg->creator == COMPONENT_UINJECT) && (PCKTSEND == fw_SendOrfw_Rcv) &&
      ...
      if (msg->l2_nextORpreviousHop.type!=ADDR_NONE) {
      	fprintf(firstAndSecondParent, "SecondPrefParent: %d ", msg->l2_nextORpreviousHop.addr_64b[7]);
   ...
   if (msg->creator == COMPONENT_UINJECT) {
		fprintf(firstAndSecondParent, "FirstPrefParent: %d\n", msg->l2_nextORpreviousHop.addr_64b[7]);
   }
   fclose(firstAndSecondParent);
   // best and second parent logs end, by Erfan
