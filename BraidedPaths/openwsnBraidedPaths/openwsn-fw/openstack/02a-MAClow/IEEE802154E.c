   ...
   */
   do { // this "loop" is only executed once
      ...
      if (ieee154e_vars.dataReceived->l1_crc==FALSE) {
         openserial_statRx(ieee154e_vars.dataReceived); // added by Erfan, Fab's stat
         // break from the do-while loop and execute abort code below
         break;
      }
      ...
      /*printf("154E: RX1(OkCRC)=> currAddr: %d, creator: %d, proto: %d, Src: %d, prevHop: %d, retriesLeft: %d, timeSlot: %d\n", 
                  idmanager_getMyID(ADDR_64B)->addr_64b[7], ieee154e_vars.dataReceived->creator, ieee154e_vars.dataReceived->l4_protocol, 
                  ieee154e_vars.dataReceived->l3_sourceAdd.addr_64b[15], ieee154e_vars.dataReceived->l2_nextORpreviousHop.addr_64b[7], 
                  ieee154e_vars.dataReceived->l2_retriesLeft, schedule_getSlotOffset());*/
      ...
      if(ieee154e_vars.deSyncTimeout > ieee154e_vars.numOfSleepSlots){
         ieee154e_vars.deSyncTimeout -= ieee154e_vars.numOfSleepSlots;
            ...
            printf("\nDesync Stat:addr: %d, have Parent: %d\n\n", 
               idmanager_getMyID(ADDR_64B)->addr_64b[7], icmpv6rpl_getPreferredParentEui64(&addres)/*icmpv6rpl_getPreferredParentIndex(&p)*/);
...
port_INLINE void activity_ti2() {
    ...
    /*printf("154E: TX=> currAddr: %d, creator: %d, proto: %d, Src: %d, prevHop: %d, retriesLeft: %d, timeSlot: %d\n", 
                  idmanager_getMyID(ADDR_64B)->addr_64b[7], ieee154e_vars.dataToSend->creator, ieee154e_vars.dataToSend->l4_protocol, 
                  ieee154e_vars.dataToSend->l3_sourceAdd.addr_64b[15], ieee154e_vars.dataToSend->l2_nextORpreviousHop.addr_64b[7], 
                  ieee154e_vars.dataToSend->l2_retriesLeft, schedule_getSlotOffset());*/

    openserial_statTx(ieee154e_vars.dataToSend); // added by Erfan, Fab's stat
...
port_INLINE void activity_tie5() {
...
#ifdef MCS_ENABLED
    // Debug messages by Erfan for MCS
  #ifdef  MCS_DEBUG    
    printf("\nBefore changing pref Parent; currentMote:%d, nextHop: %d, Reteries Left: %d\n",
            idmanager_getMyID(ADDR_64B)->addr_64b[7], 
            ieee154e_vars.dataToSend->l2_nextORpreviousHop.addr_64b[7],
    ...
    // check if (nbretries%2 == 0) then send to preferred parent else send to second preferred parent, added by Erfan for MCS
                                          // means that only if the packet is for applications
    if (ieee154e_vars.dataToSend!=NULL && (ieee154e_vars.dataToSend->creator>=COMPONENT_C6T || ieee154e_vars.dataToSend->creator==COMPONENT_FORWARDING)) {
     
       if (ieee154e_vars.dataToSend->l2_retriesLeft%2==1) {
          if (icmpv6rpl_getSecondPreferredParentEui64(&(ieee154e_vars.dataToSend->l2_nextORpreviousHop))) {
              ieee802154_header_iht ieee802514_header;
              ieee802154_retrieveHeader(ieee154e_vars.dataToSend,&ieee802514_header);
              // toss the IEEE802.15.4 header
              packetfunctions_tossHeader(ieee154e_vars.dataToSend,ieee802514_header.headerLength);
              ieee802154_prependHeader(
                  ieee154e_vars.dataToSend,
                  ieee154e_vars.dataToSend->l2_frameType,
                  FALSE,
                  ieee154e_vars.dataToSend->l2_dsn,
                  &(ieee154e_vars.dataToSend->l2_nextORpreviousHop)
              );
      #ifdef  MCS_DEBUG  
             printf("After changing pref Parent; currentMote:%d, nextHop: %d, RetLeft: %d\n", 
                     idmanager_getMyID(ADDR_64B)->addr_64b[7], 
                     ieee154e_vars.dataToSend->l2_nextORpreviousHop.addr_64b[7],
                     ieee154e_vars.dataToSend->l2_retriesLeft
                   );
       ...
       else if (ieee154e_vars.dataToSend->l2_retriesLeft!=0) {
         if (icmpv6rpl_getPreferredParentEui64(&(ieee154e_vars.dataToSend->l2_nextORpreviousHop))) {
              ieee802154_header_iht ieee802514_header;
              ieee802154_retrieveHeader(ieee154e_vars.dataToSend,&ieee802514_header);
              // toss the IEEE802.15.4 header
              packetfunctions_tossHeader(ieee154e_vars.dataToSend,ieee802514_header.headerLength);
         ...
         #ifdef  MCS_DEBUG
               printf("\nReturned to best parent: Reteries Left: %d\n", ieee154e_vars.dataToSend->l2_retriesLeft);
         #endif
         }
      }  
    }
    // checks ends by Erfan for MCS
#endif
    ...
    */
    do { // this "loop" is only executed once
        ...
        if (ieee154e_vars.ackReceived->l1_crc==FALSE) {
            openserial_statRx(ieee154e_vars.ackReceived); // added by Erfan, Fab's stat
        ...
        //we received an ack // added by Erfan, Fab's stat
...
port_INLINE void activity_ri5(PORT_TIMER_WIDTH capturedTime) {
    ...
    */
    do { // this "loop" is only executed once
        ...
        /*printf("154E: RX2(OkCRC)=> currAddr: %d, creator: %d, proto: %d, Src: %d, prevHop: %d, retriesLeft: %d, timeSlot: %d\n", 
                  idmanager_getMyID(ADDR_64B)->addr_64b[7], ieee154e_vars.dataReceived->creator, ieee154e_vars.dataReceived->l4_protocol, 
                  ieee154e_vars.dataReceived->l3_sourceAdd.addr_64b[15], ieee154e_vars.dataReceived->l2_nextORpreviousHop.addr_64b[7], 
                  ieee154e_vars.dataReceived->l2_retriesLeft, schedule_getSlotOffset());*/
        //stat event -> packet received
...
port_INLINE void activity_ri9(PORT_TIMER_WIDTH capturedTime) {
    ...
    //ack transmitted // added by Erfan, Fab's stat
