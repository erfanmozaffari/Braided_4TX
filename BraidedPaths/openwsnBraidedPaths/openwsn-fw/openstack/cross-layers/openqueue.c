...
*/
bool openqueue_isQueueEmpty(uint8_t proto, uint8_t own) {
   uint8_t i;
   ...
   for (i=0;i<QUEUELENGTH;i++) {
      if (openqueue_vars.queue[i].creator==own) {
         ...
         nextHop = openqueue_vars.queue[i].l2_nextORpreviousHop.addr_64b[7];
         forwardingPackets++;
      }
   }
   if (forwardingPackets<=1) {
      // if there is only one forwarding packet in the queue 
      if (forwardingPackets == 1) {
         // check to know that the current forwarding packet in the queue is in its last hop to the sink or not!
         // we suppose that sink is node 1
         if (nextHop==1)
            return TRUE;
         else
            return FALSE;
