...
owerror_t sixtop_request(
    ...
    if (
        ...
        if (code == IANA_6TOP_CMD_ADD || code == IANA_6TOP_CMD_RELOCATE){
           ...
           //statEvent -> to OV // added by Erfan, Fab's stat
...
void timer_sixtop_management_fired(void) {
    ...
    default:
        ...
        //sixtop_sendKA(); // commented by Erfan, as we do not need it for synchronization, other packets do the same
        break;
    }
}
...
void sixtop_six2six_sendDone(OpenQueueEntry_t* msg, owerror_t error){
    ...
    if (msg->l2_sixtop_messageType == SIXTOP_CELL_REQUEST){
        //stats to OpenVisualizer  // added by Erfan, Fab's stat
        uint8_t status;
        if(error == E_FAIL)
        status = FAILED;
        else
    ...
    if (msg->l2_sixtop_messageType == SIXTOP_CELL_RESPONSE){
        ...
        //stat to openVizualizer for the response  // added by Erfan, Fab's stat
        uint8_t status;
        if (error == E_SUCCESS)
        status = TXED;
        else
    ...
    if (type == SIXTOP_CELL_REQUEST){
        ...
        do{
            ...
            if (code == IANA_6TOP_CMD_LIST){
                ...
                for(i=0; i<maxNumCells; i++) {
                    if (
                        ...
                        //stats to OV  // added by Erfan, Fab's stat
            ...
            if (code == IANA_6TOP_CMD_DELETE){
                //stats to OV // added by Erfan, Fab's stat
                openserial_stat6Pcmd(CELLADD_REQ, RCVD, &(pkt->l2_nextORpreviousHop), NULL, 0); // added by Erfan, Fab's stat
